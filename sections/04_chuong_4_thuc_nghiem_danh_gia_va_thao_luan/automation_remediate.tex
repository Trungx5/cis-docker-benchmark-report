...
{\def\LTcaptype{none}
\begin{center}
\captionof{table}{Bảng chi tiết khắc phục tự động}\label{tab:chi-tiet-khac-phuc-tu-dong}
\end{center}
\begin{longtable}{@{}|
...
3 & 3.1 - 3.2: Quyền file service Docker. & Module \texttt{ansible.\allowbreak builtin.\allowbreak file}; xác định service file bằng \texttt{systemctl show}, sau đó đặt owner/group và mode phù hợp. & File service thuộc sở hữu root; permission là \texttt{0644} hoặc hạn chế hơn. \\
\hline
...
& 3.17 - 3.18: Khắc phục quyền \texttt{daemon.json}, sysconfig Docker và containerd socket. & Module \texttt{ansible.\allowbreak builtin.\allowbreak file}; kiểm tra file tồn tại rồi đặt owner/group/mode phù hợp. & File cấu hình Docker và containerd socket có ownership đúng; permission không quá rộng. \\
\hline

4 & 4.1: Cấu hình container chạy bằng non-root user. & Chỉnh Dockerfile hoặc cấu hình container bằng Ansible; thêm chỉ thị \texttt{USER} hoặc cấu hình user khi tạo container. & Container không chạy bằng user \texttt{root}, giảm rủi ro leo thang đặc quyền khi container bị khai thác. \\
\hline
...
5 & 5.9, 5.14: Giới hạn port và interface. & \begin{minipage}[t]{\linewidth}\raggedright
